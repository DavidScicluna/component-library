import type { CursorClassObj, PointerEventsClassObj, ResizeClassObj, ScrollBehaviorClassObj, ScrollSnapAlignClassObj, ScrollSnapStopClassObj, ScrollSnapTypeClassObj, ThemeColorObj, ThemeColorOpacityObj, ThemeSpacingObj, UserSelectClassObj, WillChangeClassObj } from '@common/types';
declare const _default: {
    cursor: CursorClassObj;
    caret_color: {
        opacity: ThemeColorOpacityObj;
        hover: ThemeColorObj;
        focus: ThemeColorObj;
        active: ThemeColorObj;
        before: ThemeColorObj;
        after: ThemeColorObj;
        gray: Record<import("@common/types").ThemeColorHue, string>;
        red: Record<import("@common/types").ThemeColorHue, string>;
        pink: Record<import("@common/types").ThemeColorHue, string>;
        purple: Record<import("@common/types").ThemeColorHue, string>;
        deep_purple: Record<import("@common/types").ThemeColorHue, string>;
        indigo: Record<import("@common/types").ThemeColorHue, string>;
        blue: Record<import("@common/types").ThemeColorHue, string>;
        light_blue: Record<import("@common/types").ThemeColorHue, string>;
        cyan: Record<import("@common/types").ThemeColorHue, string>;
        teal: Record<import("@common/types").ThemeColorHue, string>;
        green: Record<import("@common/types").ThemeColorHue, string>;
        light_green: Record<import("@common/types").ThemeColorHue, string>;
        lime: Record<import("@common/types").ThemeColorHue, string>;
        yellow: Record<import("@common/types").ThemeColorHue, string>;
        orange: Record<import("@common/types").ThemeColorHue, string>;
        deep_orange: Record<import("@common/types").ThemeColorHue, string>;
        transparent: string;
        black: string;
        white: string;
    };
    pointer_events: PointerEventsClassObj;
    resize: ResizeClassObj;
    scroll_behavior: ScrollBehaviorClassObj;
    scroll_p: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_px: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_py: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_pt: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_pr: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_pb: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_pl: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_m: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_mx: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_my: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_mt: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_mr: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_mb: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_ml: {
        hover: ThemeSpacingObj;
        focus: ThemeSpacingObj;
        active: ThemeSpacingObj;
        before: ThemeSpacingObj;
        after: ThemeSpacingObj;
        0: string;
        1: string;
        2: string;
        3: string;
        4: string;
        5: string;
        6: string;
        0.25: string;
        0.5: string;
        0.75: string;
        1.25: string;
        1.5: string;
        1.75: string;
        2.25: string;
        2.5: string;
        2.75: string;
        3.25: string;
        3.5: string;
        3.75: string;
        4.25: string;
        4.5: string;
        4.75: string;
        5.25: string;
        5.5: string;
        5.75: string;
        6.25: string;
        6.5: string;
        6.75: string;
        7: string;
        7.25: string;
        7.5: string;
        7.75: string;
        8: string;
        8.25: string;
        8.5: string;
        8.75: string;
        9: string;
        9.25: string;
        9.5: string;
        9.75: string;
        10: string;
        10.25: string;
        10.5: string;
        10.75: string;
        11: string;
        11.25: string;
        11.5: string;
        11.75: string;
        12: string;
        12.25: string;
        12.5: string;
        12.75: string;
        13: string;
        13.25: string;
        13.5: string;
        13.75: string;
        14: string;
        14.25: string;
        14.5: string;
        14.75: string;
        15: string;
        15.25: string;
        15.5: string;
        15.75: string;
        16: string;
        16.25: string;
        16.5: string;
        16.75: string;
        17: string;
        17.25: string;
        17.5: string;
        17.75: string;
        18: string;
        18.25: string;
        18.5: string;
        18.75: string;
        19: string;
        19.25: string;
        19.5: string;
        19.75: string;
        20: string;
        20.25: string;
        20.5: string;
        20.75: string;
        21: string;
        21.25: string;
        21.5: string;
        21.75: string;
        22: string;
        22.25: string;
        22.5: string;
        22.75: string;
        23: string;
        23.25: string;
        23.5: string;
        23.75: string;
        24: string;
        24.25: string;
        24.5: string;
        24.75: string;
    };
    scroll_snap_align: ScrollSnapAlignClassObj;
    scroll_snap_stop: ScrollSnapStopClassObj;
    scroll_snap_type: ScrollSnapTypeClassObj;
    user_select: UserSelectClassObj;
    will_change: WillChangeClassObj;
};
export default _default;
//# sourceMappingURL=interactivity.d.ts.map