...
import type { PolymorphicDefaultElement, PolymorphicMouseEvent, ResponsiveValue, ThemeAppAppearanceProps, ThemeRadius, ThemeSpacing } from '@common/types';
...
export type CardProps<Element extends ElementType = PolymorphicDefaultElement> = BoxProps<Element, CardOtherProps<Element>>;
export type CardRef<Element extends ElementType = PolymorphicDefaultElement> = BoxRef<Element>;
export type CardContext<Element extends ElementType = PolymorphicDefaultElement> = Pick<CardProps<Element>, 'color' | 'colorMode' | 'isCollapsable' | 'isDisabled' | 'isDivisible' | 'isOpen' | 'spacing' | 'variant'> & {
    isHovering: boolean;
